...
enum class Message(val path: String) {
    ...
    fun sendMessage(player: Player?, component: PlaceHolderObj?) {
        ...
        var messageList: MutableList<String>? = fileManager.getStringListFromFile(FileType.MESSAGE, this.path)
        if (messageList.isNullOrEmpty())
            messageList = mutableListOf(fileManager.getStringFromFile(FileType.MESSAGE, this.path) ?: return)
    ...
    private fun buildMessageComponent(messageList: MutableList<String>, component: PlaceHolderObj) {
        messageList.indices.forEach { i ->
            if (messageList[i].contains('%')) {
                component.values.indices.forEach { x ->
                    messageList[i] = messageList[i].replace(component.compoEnum[x].placeHolder, component.values[x])
                }
            }
        }
    }
...
enum class PlaceHolder(val placeHolder: String) {
    GAME("%game%"),
    PLAYER("%player%"),
    TIME("%time%");
}

class PlaceHolderObj(val compoEnum: List<PlaceHolder>, val values: List<String>)